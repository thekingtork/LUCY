//ESTA VARIAVLE FUNCIONA COMO CONTADOR DE TOMAS
var cantidad_tomas = 0;
var cantidad_pag = 0;
var cantidad_bene = 0;
var talla1 = 0;
var peso1 = 0;
...
var C_fn_Correg;
var _contador = 0;
//ESTA FUNCION SE EJECUTA CUANDO SOLO ENCUENTRA 1 PESOS
...
//esta es la matriz para la talla inicial de las niñas
...
//ESTRA FUNCION CALCULA LOS MESES DE VIDA
...
//ESTRA FUNCION GENERA LAS TALLAS
function generar_talla (sexo,fNac) {
    if (sexo == "F" || sexo == "f") {
      var meses_vividos = Math.floor((restaFechas(fNac,'05/02/2015')/365)*12);
        if (meses_vividos<=72) {
          //console.log('Los Meses que ha vivido son: '+meses_vividos+' - '+talla_ninas[meses_vividos][Math.floor(Math.random()*3 + 2)]+' y el sexo es '+sexo);
          talla1 = talla_ninas[meses_vividos][Math.floor(Math.random()*3 + 2)];
        //  alert('femenino: '+talla1);
        };
    }
    else if (sexo == "M" || sexo == "m") {
      var meses_vividos = Math.floor((restaFechas(fNac,'05/02/2015')/365)*12);
      if (meses_vividos<=72) {
          //console.log('Los Meses que ha vivido son: '+meses_vividos+" - "+talla_ninos[meses_vividos][Math.floor(Math.random()*3 + 2)]+' y el sexo es '+sexo);
          talla1 = talla_ninos[meses_vividos][Math.floor(Math.random()*3 + 2)];
        //  alert('masculino: '+talla1);
        };
    }
    else{
 ...
 //ESTRA FUNCION GENERA LOS PESOS
function generar_peso (sexo,tall) {
	//alert ('generando datos con: '+sexo+tall);
     if (sexo == "F" || sexo == "f") {
     var talla_corr = parseInt(tall);
      if (talla_corr <=127) {
          //console.log('Los Meses que ha vivido son: '+meses_vividos+' - '+talla_ninas[meses_vividos][Math.floor(Math.random()*3 + 2)]+' y el sexo es '+sexo);
          peso1 = peso_ninas[talla_corr][Math.floor(Math.random()*3 + 2)];
         // alert(peso1);
        };
    }
    else if (sexo == "M" || sexo == "m") {
    var talla_corr = parseInt(tall);
    //alert('con esta talla entrara'+talla_corr);
      if (talla_corr <=127) {
          //console.log('Los Meses que ha vivido son: '+meses_vividos+" - "+talla_ninos[meses_vividos][Math.floor(Math.random()*3 + 2)]+' y el sexo es '+sexo);
          peso1 = peso_ninos[talla_corr][Math.floor(Math.random()*3 + 2)];
         // alert(peso1);
        };
    }
    else{
...
// ESTA FUNCION SE EJECUTA SOLO CUANDO EL USUARIO NO CUENTA CON NINGUNA TOMA
function todos_pesos (){
var pesoAux = 0;
var tallaAux = 0;
var codigo = "CODE:\n";
	codigo += "TAG POS=1 TYPE=IMG ATTR=SRC:https://rubonline.icbf.gov.co/Image/btn/add.gif"+"\n";
	codigo += "WAIT SECONDS=1"+"\n";
	codigo += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:form1 ATTR=ID:cphCont_txtPeso CONTENT="+peso1+"\n";///AQUI DEBO INGRESAR EL PRIMER PESO 
	codigo += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:form1 ATTR=ID:cphCont_txtTalla CONTENT="+talla1+"\n";// AQUI DEBO INGRESAR LA PRIMERA TALLA
	...
	codigo += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:form1 ATTR=ID:cphCont_cuwFechaValoracionNuricional_txtFecha CONTENT=05/02/2014"+"\n";
...
function toma1(){
	...
	macro += "SET !VAR4 EVAL(\"var talla3={{!VAR2}};var aumentot4=Math.floor(Math.random()*3 + 1)+Math.random();var talla4=talla3+aumentot4; talla4.toFixed(2);\")"+"\n";
	macro += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:form1 ATTR=ID:cphCont_txtTalla CONTENT={{!VAR4}}"+"\n";
	macro += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:form1 ATTR=ID:cphCont_cuwFechaValoracionNuricional_txtFecha CONTENT=05/05/2014"+"\n";
	macro += "TAG POS=1 TYPE=IMG ATTR=SRC:https://rubonline.icbf.gov.co/Image/btn/save.gif"+"\n";
	macro += "WAIT SECONDS=0.5"+"\n";
	macro += "TAG POS=1 TYPE=IMG ATTR=SRC:https://rubonline.icbf.gov.co/Image/btn/add.gif"+"\n";
	// CALCULO Y GUARDO PESO 3
	macro += "SET !VAR5 EVAL(\"var peso2={{!VAR3}};var aumento3=Math.random()+Math.random();var peso3=peso2+aumento3; peso3.toFixed(2);\")"+"\n";
	macro += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:form1 ATTR=ID:cphCont_txtPeso CONTENT={{!VAR5}}"+"\n";
	// CALCULO Y GUARDO TALLA 3
	macro += "SET !VAR6 EVAL(\"var talla2={{!VAR4}};var aumentot3=Math.floor(Math.random()*3 + 1)+Math.random();var talla3=talla2+aumentot3; talla3.toFixed(2);\")"+"\n";
	macro += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:form1 ATTR=ID:cphCont_txtTalla CONTENT={{!VAR6}}"+"\n";
	macro += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:form1 ATTR=ID:cphCont_cuwFechaValoracionNuricional_txtFecha CONTENT=05/08/2014"+"\n";
	macro += "TAG POS=1 TYPE=IMG ATTR=SRC:https://rubonline.icbf.gov.co/Image/btn/save.gif"+"\n";
	macro += "WAIT SECONDS=1"+"\n";
	macro += "TAG POS=1 TYPE=IMG ATTR=SRC:https://rubonline.icbf.gov.co/Image/btn/add.gif"+"\n";
	// CALCULO Y GUARDO PESO 4
	macro += "SET !VAR7 EVAL(\"var peso2={{!VAR5}};var aumento3=Math.random()+Math.random();var peso3=peso2+aumento3; peso3.toFixed(2);\")"+"\n";
	macro += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:form1 ATTR=ID:cphCont_txtPeso CONTENT={{!VAR7}}"+"\n";
	// CALCULO Y GUARDO TALLA 4
	macro += "SET !VAR8 EVAL(\"var talla2={{!VAR6}};var aumentot3=Math.floor(Math.random()*2+1);var talla3=talla2+aumentot3; talla3.toFixed(2);\")"+"\n";
	macro += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:form1 ATTR=ID:cphCont_txtTalla CONTENT={{!VAR8}}"+"\n";
	macro += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:form1 ATTR=ID:cphCont_cuwFechaValoracionNuricional_txtFecha CONTENT=05/11/2014"+"\n";
	macro += "TAG POS=1 TYPE=IMG ATTR=SRC:https://rubonline.icbf.gov.co/Image/btn/save.gif"+"\n";
	macro += "WAIT SECONDS=1.2"+"\n";
	macro += "TAG POS=1 TYPE=IMG ATTR=SRC:https://rubonline.icbf.gov.co/Image/btn/list.png"+"\n";
	iimPlay(macro);
}
//ESTA FUNCION SE EJECUTA CUANDO SOLO ENCUENTRA 2 PESOS
function peso2(){
	var macro = "CODE:\n";
	macro += "SET !EXTRACT_TEST_POPUP NO"+"\n";
	macro += "TAG POS=1 TYPE=TH ATTR=TXT:*"+"\n";
	macro += "TAG POS=R16 TYPE=TD ATTR=TXT:* EXTRACT=TXT"+"\n";
	//VAR1 GUARDA EL PESO 2
	macro += "SET !VAR1 {{!EXTRACT}}"+"\n";
	macro += "SET !EXTRACT NULL"+"\n";
	macro += "TAG POS=1 TYPE=TD ATTR=TXT:{{!VAR1}}"+ "\n";
	macro += "TAG POS=R1 TYPE=TD ATTR=TXT:* EXTRACT=TXT"+"\n";
	//VAR2 GUARDA LA TALLA 2
	macro += "SET !VAR2 {{!EXTRACT}}"+"\n";
	macro += "TAG POS=1 TYPE=IMG ATTR=SRC:https://rubonline.icbf.gov.co/Image/btn/add.gif"+"\n";
	macro += "WAIT SECONDS = 1"+"\n";
	// CALCULO Y GUARDO PESO 3
	...
	macro += "TAG POS=1 TYPE=IMG ATTR=SRC:https://rubonline.icbf.gov.co/Image/btn/add.gif"+"\n";
	// CALCULO Y GUARDO PESO 4
	macro += "SET !VAR1 EVAL(\"var peso2={{!VAR3}};var aumento3=Math.random()+Math.random();var peso3=peso2+aumento3; peso3.toFixed(2);\")"+"\n";
	macro += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:form1 ATTR=ID:cphCont_txtPeso CONTENT={{!VAR1}}"+"\n";
	// CALCULO Y GUARDO TALLA 4
	macro += "SET !VAR6 EVAL(\"var talla2={{!VAR4}};var aumentot3=Math.floor(Math.random());var talla3=talla2+aumentot3; talla3.toFixed(2);\")"+"\n";
	macro += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:form1 ATTR=ID:cphCont_txtTalla CONTENT={{!VAR6}}"+"\n";
	macro += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:form1 ATTR=ID:cphCont_cuwFechaValoracionNuricional_txtFecha CONTENT=05/11/2014"+"\n";
	macro += "TAG POS=1 TYPE=IMG ATTR=SRC:https://rubonline.icbf.gov.co/Image/btn/save.gif"+"\n";
...
//ESTA FUNCION SE EJECUTA CUANDO SOLO ENCUENTRA 3 PESOS
function peso4(){
	var macro = "CODE:\n";
	macro += "SET !EXTRACT_TEST_POPUP NO"+"\n";
	macro += "TAG POS=1 TYPE=TH ATTR=TXT:*"+"\n";
	macro += "TAG POS=R25 TYPE=TD ATTR=TXT:* EXTRACT=TXT"+"\n";
	macro += "SET !VAR1 {{!EXTRACT}}"+"\n";
	macro += "SET !EXTRACT NULL"+"\n";
	macro += "TAG POS=1 TYPE=TD ATTR=TXT:{{!VAR1}}"+ "\n";
	macro += "TAG POS=R1 TYPE=TD ATTR=TXT:* EXTRACT=TXT"+"\n";
	...
	macro += "SET !VAR3 EVAL(\"var peso3={{!VAR1}};var aumento4=Math.floor(Math.random()*1 + 1)+Math.random();var peso4=peso3+aumento4; peso4.toFixed(2);\")"+"\n";
	macro += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:form1 ATTR=ID:cphCont_txtPeso CONTENT={{!VAR3}}"+"\n";
	macro += "SET !VAR4 EVAL(\"var talla3={{!VAR2}};var aumentot4=Math.floor(Math.random()*1 + 1)+Math.random();var talla4=talla3+aumentot4; talla4.toFixed(2);\")"+"\n";
	macro += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:form1 ATTR=ID:cphCont_txtTalla CONTENT={{!VAR4}}"+"\n";
	macro += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:form1 ATTR=ID:cphCont_cuwFechaValoracionNuricional_txtFecha CONTENT=05/11/2014"+"\n";
	macro += "TAG POS=1 TYPE=IMG ATTR=SRC:https://rubonline.icbf.gov.co/Image/btn/save.gif"+"\n";
	macro += "WAIT SECONDS=0.5"+"\n";
...
//LA FUNCION PAGINADOR RECUERDA EN QUE PAGINA VA EL PROCESO
function paginador (pag){
	if(pag < 11){
		var pagina = "CODE:\n";
		    pagina += "SET !TIMEOUT_STEP 2"+"\n";
...
//LA FUNCION ITEN_USER INGRESA AL USUARIO INMEDIATAMENTE ANTERIOR FUNCIONANDO COMO UN CONTADOR
function item_user(numero){
	var codigo = "CODE:\n";
		    codigo += "TAG POS=1 TYPE=INPUT:IMAGE FORM=ID:form1 ATTR=ID:cphCont_gvBeneficiarios_btnInfo_"+numero+"\n";
		    codigo += "WAIT SECONDS= 2"+"\n";
	iimPlay(codigo);
}
//ESTA FUNCION ES PARA CAPTURAR LA CANTIDAD DE TOMAS POR USUARIO
function validar_datos (pos){
		var codigo = "CODE:\n";
...
function datos_ini (){
		...
		cantidad_bene = prompt('Ingresa el numero de BENEFICIARIOS','');
		
}


function capturar_sexo_fecha (pos){
//alert(pos);
	if(pos==0){	
	_contador = 3;
	alert('possigual a '+pos+' contador es igaual a '+_contador);
	var codigo = "CODE:\n";
		codigo += "SET !TIMEOUT_STEP 2"+"\n";
		codigo += "TAG POS=1 TYPE=TH ATTR=TXT:*"+"\n";
		codigo += "TAG POS=R"+_contador+" TYPE=TD ATTR=TXT:* EXTRACT=TXT"+"\n";
		codigo += "SET !VAR1 {{!EXTRACT}}"+"\n";
		...
		codigo += "TAG POS=1 TYPE=INPUT:TEXT FORM=ID:form1 ATTR=ID:cphCont_txtIdentificacion CONTENT={{!VAR1}}"+"\n";
      ...
      codigo += "PROMPT {{!EXTRACT}}"+"\n";
		codigo += "SET !VAR3 {{!EXTRACT}}"+"\n";
		codigo += "TAB CLOSE "+"\n";
		codigo += "TAB T=1"+"\n";
	iimPlay(codigo);
	C_sex = iimGetLastExtract(1);//SEXO
	//alert(C_sex);
	var C_fn = iimGetLastExtract(2);
	C_fn_Correg = C_fn.substring(0, C_fn.length-9);//FECHA DE NACIMIENTO
	}
	else{
	_contador+=8;
	//alert('valor de pos igual a '+pos+' contador es igual a '+_contador);
	var codigo = "CODE:\n";
		codigo += "TAG POS=1 TYPE=TH ATTR=TXT:*"+"\n";
		codigo += "TAG POS=R"+_contador+" TYPE=TD ATTR=TXT:* EXTRACT=TXT"+"\n";
		codigo += "SET !VAR1 {{!EXTRACT}}"+"\n";
		...
		codigo += "TAB T=1"+"\n";
	iimPlay(codigo);
	C_sex = iimGetLastExtract(1);
	//alert(C_sex);
	var C_fn = iimGetLastExtract(2);
	C_fn_Correg = C_fn.substring(0, C_fn.length-9);
	//alert(C_fn_Correg);
	}
	
}
 
function manejo_cont (){
	if(_contador==0){
		_contador+=3;
	}
	else{
...
function nucleo_peso (c_pag,c_bene){	
	for ( var i = 1; i <= c_pag ; i++){
		iimSet("i", i);
		iimDisplay(i);
		paginador(i);
	if (c_bene > 9){
		//alert('bene es mayor que 9:  '+c_bene);
		for ( var k = 0; k <= 9 ; k++){
			iimSet("k", k);
			iimDisplay(k);
			validar_datos(k+1);
			switch (cantidad_tomas) {
				case "0":
       					//alert('EL USUARIO NO TIENE TOMA');
       					capturar_sexo_fecha(k);
       					item_user(k);
       					generar_talla(C_sex,C_fn_Correg);
       					generar_peso(C_sex,talla1);
       					todos_pesos();
       					paginador(i);
       					c_bene -=1;
       				break
	...
	else if(c_bene <= 9 && c_bene > 0 ){
		//alert('bene es menor que 9 : '+c_bene);
			for ( var k = 0; k <= c_bene; k++){
			iimSet("k", k);
			iimDisplay(k);
			validar_datos(k+1);
			switch (cantidad_tomas) {
...
//2380700095902
datos_ini();
nucleo_peso(cantidad_pag,cantidad_bene);